...
def test_authorize_dashboard_nologin():
    with pytest.raises(expected_exception=AttributeError):
        main.render_template("dashboard.html")

def test_authorize_reports_nologin():
    with pytest.raises(expected_exception=AttributeError):
        main.render_template("reports.html")

def test_authorize_settings_nologin():
    with pytest.raises(expected_exception=AttributeError):
        main.render_template("settings.html")

def test_authorize_settingsNotif_nologin():
    with pytest.raises(expected_exception=AttributeError):
        main.render_template("settingNotifications.html")

def test_authorize_settingsWatch_nologin():
    with pytest.raises(expected_exception=AttributeError):
        main.render_template("settingWatchList.html")

def test_authorize_settingsPlaytime_nologin():
    with pytest.raises(expected_exception=AttributeError):
        main.render_template("settingPlaytimeTracking.html")

def test_authorize_settingsSteam_nologin():
    with pytest.raises(expected_exception=AttributeError):
        main.render_template("settingSteamAccount.html")
